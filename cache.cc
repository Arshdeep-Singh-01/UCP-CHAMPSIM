...
vector<uint8_t> allocated(LLC_WAY);
vector<uint32_t> par_way(NUM_CPUS);
uint8_t is_hit=false;

uint32_t get_set_for_llc(uint64_t address)
{
...
void initialize_partition()
{
  ...
  for(int i=0;i<NUM_CPUS;i++){
    cout<<par_way[i]<<": ";
    for(int j=0;j<ways_partition[i].size();j++){
      cout<<ways_partition[i][j]<<" ";
    }
  }
  cout<<"\n";
}
...
void get_partition()
{
  ...
  if (NUM_CPUS == 2)
  {
    uint32_t max_utility = Ut(0, 1) + Ut(1, LLC_WAY - 1);
    uint32_t temp = max_utility;
    for (int i = 2; i < LLC_WAY; i++)
    {
      temp = Ut(0, i) + Ut(1, LLC_WAY - i);
      if (temp > max_utility)
      {
        par_way[0] = i;
        max_utility = temp;
    ...
    for (int i = 0; i < NUM_CPUS; i++)
    {
      for (int k = 0; k < par_way[i]; k++)
      {
        ways_partition[i].push_back(j++);
      }
    }*/
  }
  else if (NUM_CPUS == 4)
  {
    par_way.clear();
    uint32_t max_utility = Ut(0, 1) + Ut(1, 1) + Ut(2, 1) + Ut(3, LLC_WAY - 3);
    par_way[0] = 1;
    par_way[1] = 1;
    par_way[2] = 1;
    par_way[3] = LLC_WAY - 3;
    for (uint32_t i = 1; i <= LLC_WAY - 3; i++)
    {
      for (uint32_t j = 1; j <= LLC_WAY - 2 - i; j++)
      {
        for (uint32_t k = 1; k <= LLC_WAY - 1 - i - j; k++)
        {
          uint32_t l = LLC_WAY - i - j - k;
          uint32_t a;
          if ((a = Ut(0, i) + Ut(1, j) + Ut(2, k) + Ut(3, l)) > max_utility)
          {
            max_utility = a;
            par_way[0] = i;
            par_way[1] = j;
  ...
  {
    par_way.clear();
    uint32_t max_utility = Ut(0, 1) + Ut(1, 1) + Ut(2, 1) + Ut(3, 1) + Ut(4, 1) + Ut(5, 1) + Ut(6, 1) + Ut(7, LLC_WAY - 7);
    ...
    for (uint32_t i = 1; i <= LLC_WAY - 7; i++)
    {
      for (uint32_t j = 1; j <= LLC_WAY - 6 - i; j++)
      {
        for (uint32_t k = 1; k <= LLC_WAY - 5 - i - j; k++)
        {
          for (uint32_t l = 1; l <= LLC_WAY - 4 - i - j - k; l++)
          {
            for (uint32_t m = 1; m <= LLC_WAY - 3 - i - j - k - l; m++)
            {
              for (uint32_t n = 1; n <= LLC_WAY - 2 - i - j - k - l - m; n++)
              {
                for (uint32_t o = 1; o <= LLC_WAY - 1 - i - j - k - l - m - n; o++)
                {
                  uint32_t p = LLC_WAY - i - j - k - l - m - n - o;
                  uint32_t a;
                  if ((a = Ut(0, i) + Ut(1, j) + Ut(2, k) + Ut(3, l) + Ut(4, m) + Ut(5, n) + Ut(6, o) + Ut(7, p)) > max_utility)
                  {
                    max_utility = a;
                    par_way[0] = i;
                    par_way[1] = j;
...
void CACHE::handle_fill()
{
  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
  {
    ...
    if (cache_type == IS_L2C && sample_sets.find(set) != sample_sets.end())
    {
      uint32_t set1 = get_set_for_llc(MSHR.entry[mshr_index].address);
      atds[cpu].deal_with_new_block(set1, MSHR.entry[mshr_index].address);
    }
...
int CACHE::check_hit(PACKET *packet)
{
  ...
  if (cache_type == IS_LLC && par_way[cpu] != ways_partition[cpu].size())
  {
    uint32_t way;
    for (uint32_t i = 0; i < ways_partition[cpu].size(); i++)
    {
    ...
    for (int i = 0; i < allocated.size(); i++)
    {
      if (allocated[i] == false)
      {
        if (block[set][i].valid && (block[set][i].tag == packet->address))
        {
          match_way = i;
          //allocated[i] = true;
          ways_partition[cpu].push_back(i);
          is_hit = true;
          DP(if (warmup_complete[packet->cpu])
           {
